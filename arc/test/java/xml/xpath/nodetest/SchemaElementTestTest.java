 ...
 * SchemaElementTest is not found in the in-scope element declarations, a 
 ...
 *   declaration of customer is not abstract; the type annotation of the 
 *   candidate node is the same as or derived from the schema type declared in 
 *   the customer element declaration; and either the candidate node is not 
 *   nilled, or customer is declared to be nillable.
 * - customer is a top-level element declaration in the in-scope element 
 ...
 *   customer; the type annotation of the candidate node is the same as or 
 *   derived from the schema type declared for the client element; and either 
 *   the candidate node is not nilled, or client is declared to be nillable.
 */
public class SchemaElementTestTest {
    @Test
    public void valid_schema_element_test_is_recognized() {
        assertTrue(SchemaElementTest.isSchemaElementTest("schema-element(customer)"));
    }

    @Test
    public void empty_schema_element_test_is_not_recognized() {
        assertFalse(SchemaElementTest.isSchemaElementTest("schema-element()"));
    }

    @Test
    public void schema_element_test_containing_string_is_not_recognized() {
        assertFalse(SchemaElementTest.isSchemaElementTest("schema-element(\"customer\")"));
        assertFalse(SchemaElementTest.isSchemaElementTest("schema-element('customer')"));
    }
}